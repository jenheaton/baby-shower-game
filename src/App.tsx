 ...
 * - Host creates a room, players join via link/QR, guess prices.
 * - “Closest without going over” (or closest overall) scoring.
 * - Live player list, reveals for guests, and scoreboard.
 *
 * This version fixes:
 * - Lobby player list not updating
 * - Add Item reliability
 * - Scores updating + host Game view shows players/scores
 * - Guests see revealed price, their guess, diff, and their score
 * - Added “How to Play” for guests
 */
...
export default function App() {
  ...
  const [roomCode, setRoomCode] = useState<string>(new URLSearchParams(location.search).get("room") || "");
  ...
  return (
    <div className="grid gap-6 md:gap-8 md:grid-cols-2">
      <div className="bg-white/70 rounded-2xl p-5 md:p-8 shadow">
        ...
        <p className="text-sm opacity-80 mb-4">Create a room, add items + real prices, then share a link.</p>
        <button className="px-4 py-2 rounded-2xl shadow bg-rose-500 text-white hover:bg-rose-600" onClick={() => setRole("host")}>
          Start as Host
        </button>
      </div>
      <div className="bg-white/70 rounded-2xl p-5 md:p-8 shadow">
        <h2 className="text-xl font-semibold mb-2">Join a game</h2>
        <p className="text-sm opacity-80 mb-4">Enter the 4–6 digit room code from the host.</p>
        <div className="flex items-center gap-2">
          <input value={roomCode} onChange={(e) => setRoomCode(e.target.value.toUpperCase())} placeholder="ROOM" className="flex-1 px-3 py-2 rounded-xl border bg-white" />
          <button className="px-4 py-2 rounded-2xl shadow bg-indigo-500 text-white hover:bg-indigo-600 disabled:opacity-40" disabled={joinDisabled} onClick={() => setRole("player")}>
            Join
          </button>
        </div>
      </div>
...
function TabButton({ active, children, onClick, disabled }: any) {
  return (
    <button disabled={disabled} onClick={onClick} className={`${active ? "bg-slate-900 text-white" : "bg-white/70 hover:bg-white"} px-4 py-2 rounded-2xl text-sm shadow ${disabled ? "opacity-40 cursor-not-allowed" : ""}`}>
      {children}
    </button>
  );
}
...
function HostView({ roomCode: initial, setRoomCode }: { roomCode: string; setRoomCode: (c: string) => void }) {
  ...
  useEffect(() => {
    ...
    // IMPORTANT: Do NOT wipe players/items when initializing the room
    (async () => {
      const now = Date.now();
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-8 p-4 bg-white/70 rounded-2xl text-xs">
        Share link: <code className="bg-slate-100 px-2 py-1 rounded">{location.origin + location.pathname}?room={room.code}</code>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="mt-4 grid gap-4 md:grid-cols-2">
      ...
      <div className="bg-white/70 rounded-2xl p-4 shadow">
        ...
        <div className="mt-4 text-xs opacity-70">Players can open the link on their phones to join instantly.</div>
      </div>
    </div>
  );
}
...
function HostItems({ room }: { room: RoomState }) {
  ...
  const addItem = async () => {
    const v = numFromInput(draft.price);
    if (!ensureFirebase() || !draft.name || v == null) return alert("Need item name and price (e.g., 28.99)");
    try {
      const item: Item = { id: uuidv4(), name: draft.name.trim(), price: v, imageUrl: draft.imageUrl || undefined, note: draft.note || undefined };
  ...
  const setRule = async (rule: Rule) => await update(ref(db, `rooms/${room.code}`), { rule });
  const setRoundTime = async (sec: number) => await update(ref(db, `rooms/${room.code}`), { roundDurationSec: sec });

  const loadSampleItems = async () => {
    if (!ensureFirebase()) return;
    const samples: Item[] = SAMPLE_ITEMS.map((it) => ({ ...it, id: uuidv4() }));
    await update(ref(db, `rooms/${room.code}`), { items: [...(room.items || []), ...samples] });
  };
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-3">
      ...
      <div className="bg-white/70 rounded-2xl p-4 shadow">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <button onClick={loadSampleItems} className="px-3 py-2 rounded-xl bg-emerald-600 text-white">Load sample items (25)</button>
          <button onClick={clearAll} className="px-3 py-2 rounded-xl bg-rose-200">Clear all items</button>
        </div>
...
function HostGame({ room }: { room: RoomState }) {
  ...
  const roundGuesses = (room.guesses?.[String(room.roundIndex)] || {}) as Record<string, Guess>;
  const playerCount = Object.keys(room.players || {}).length;
  const submittedCount = Object.keys(roundGuesses).length;

  const startRound = async () => {
    if (!ensureFirebase() || !currentItem) return;
    await update(ref(db, `rooms/${room.code}`), { status: "in_round", roundEndsAt: Date.now() + room.roundDurationSec * 1000 });
  };
  const reveal = async () => {
    if (!ensureFirebase() || !currentItem) return;
    ...
    if (entries.length) {
      if (room.rule === "closest_without_over") {
        ...
        winner = pool.reduce((best, g) => (Math.abs(g.value - currentItem.price) < Math.abs(best.value - currentItem.price) ? g : best), pool[0]);
      } else {
        winner = entries.reduce((best, g) => (Math.abs(g.value - currentItem.price) < Math.abs(best.value - currentItem.price) ? g : best), entries[0]);
      }
    }
  ...
  const next = async () => {
    ...
    if (nextIndex >= (room.items?.length || 0)) {
      ...
      await update(ref(db, `rooms/${room.code}`), { status: "lobby", roundIndex: nextIndex, roundEndsAt: null });
    }
  };

  if (!currentItem) return <div className="mt-4 p-4 bg-white/70 rounded-2xl">Add items to start.</div>;

  // Build a live player list with "has guessed" indicator and scores
  ...
  const rows = Object.entries(room.scores || {}).map(([pid, score]) => ({
    playerId: pid,
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-3">
      ...
      <div className="bg-white/70 rounded-2xl p-4 shadow md:col-span-2">
        ...
        <div className="mt-4 flex gap-2">
          ...
          {room.status === "revealed" && <button className="px-4 py-2 rounded-xl bg-indigo-600 text-white" onClick={next}>Next item</button>}
          {room.status === "finished" && <div className="px-3 py-2 rounded-xl bg-yellow-100">Game finished • See Scores tab</div>}
        </div>
      </div>
          ...
          <ul className="text-sm divide-y">
            {players.map((p) => (
              <li key={p.id} className="py-1 flex justify-between">
                <span>{p.name}</span>
                <span className={`text-xs ${p.guessed ? "text-emerald-700" : "opacity-60"}`}>{p.guessed ? "✓ guessed" : "—"}</span>
              </li>
            ))}
        ...
        <div className="bg-white/70 rounded-2xl p-4 shadow">
          ...
          {!rows.length ? (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {rows.map((r, idx) => (<tr key={r.playerId} className="border-t"><td className="py-1">{idx + 1}</td><td>{r.name}</td><td>{r.score}</td></tr>))}
              </tbody>
            </table>
          )}
        </div>
...
function WinnerCard({ room }: { room: RoomState }) {
  ...
  const winner = pool.reduce((best, g) => (Math.abs(g.value - item.price) < Math.abs(best.value - item.price) ? g : best), pool[0]);
  return (
    <div className="mt-3 p-3 rounded-xl bg-emerald-50 border border-emerald-200">
      ...
      <div className="text-sm opacity-70">with ${winner.value.toFixed(2)} (diff {(Math.abs(winner.value - item.price)).toFixed(2)})</div>
    </div>
  );
}
...
function PlayerView({ roomCode: initial, setRoomCode }: { roomCode: string; setRoomCode: (c: string) => void }) {
  ...
  const join = async () => {
    ...
    await update(ref(db, `rooms/${trimmed}/players/${playerId}`), { id: playerId, name: name.trim(), joinedAt: Date.now() });
    setCode(trimmed);
  };

  const submitGuess = async () => {
    if (!ensureFirebase() || !room) return;
    const v = numFromInput(guess);
    if (v == null) return alert("Enter a number (e.g., 23.50)");
    const roundKey = String(room.roundIndex);
    await update(ref(db, `rooms/${room.code}/guesses/${roundKey}/${playerId}`), { playerId, playerName: name, value: v });
    setGuess("");
  };

  const currentItem = room?.items?.[room?.roundIndex || 0];
  const roundGuesses = (room?.guesses?.[String(room?.roundIndex || 0)] || {}) as Record<string, Guess>;
  const myGuess = roundGuesses?.[playerId]?.value;
  const myScore = (room?.scores || {})[playerId] || 0;
  const diff = currentItem && typeof myGuess === "number" ? Math.abs(myGuess - currentItem.price) : null;
    ...
    <div className="mt-6 max-w-xl mx-auto bg-white/70 rounded-2xl p-5 shadow">
      {!room && (
        <div>
          ...
          <div className="mt-3 text-xs opacity-70">Ask the host for the room code or open their invite link.</div>
        </div>
      )}
      ...
      {room && (
        <div>
          ...
          <div className="mt-3 p-3 rounded-xl bg-rose-50">
            <div className="text-sm opacity-60">Round {room.roundIndex + 1} / {room.items.length}</div>
            <div className="text-lg font-semibold">{currentItem?.name || "Waiting for host…"}</div>
            {room.status === "in_round" && <div className="mt-1 text-sm">Ends in <Countdown targetMs={room.roundEndsAt || 0} /></div>}
          </div>
          ...
          {room.status === "in_round" && (
            <div className="mt-3 flex gap-2">
              ...
              <button className="px-4 py-2 rounded-xl bg-emerald-600 text-white" onClick={submitGuess} disabled={typeof myGuess === "number"}>
          ...
          {room.status === "revealed" && currentItem && (
            <div className="mt-4 p-3 rounded-xl bg-emerald-50 border border-emerald-200">
              ...
              <div className="text-sm">Correct price: <span className="font-bold">${currentItem.price.toFixed(2)}</span></div>
              {typeof myGuess === "number" ? (
                <>
                  <div className="text-sm">Your guess: ${myGuess.toFixed(2)}</div>
                  <div className="text-sm">Off by: {diff!.toFixed(2)}</div>
                </>
              ) : (
                <div className="text-sm opacity-70">You didn’t submit a guess this round.</div>
              )}
              <div className="text-sm mt-1">Your score: <span className="font-bold">{myScore}</span></div>
            </div>
          )}

          {room.status !== "in_round" && room.status !== "revealed" && <div className="mt-3 text-sm opacity-60">Waiting for next round…</div>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="text-xs bg-white rounded-xl border p-3 mb-3">
      ...
      <ul className="list-disc pl-4 space-y-1">
        ...
        <li>When an item appears, type the price you think it costs (e.g., <code>24.99</code>) and tap <b>Submit</b>.</li>
        <li>Scoring: <b>Closest without going over</b> wins 1 point. (If everyone goes over, closest overall wins.)</li>
        <li>After each round, you’ll see the <b>correct price</b>, your <b>guess</b>, how far off you were, and your <b>score</b>.</li>
      </ul>
    </div>
  );
}
